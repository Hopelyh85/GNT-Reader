...
function convert() {
  ...
  // Find all books
  const bookRegex = /<book id="([^"]+)">[\s\S]*?<title>([^<]+)<\/title>([\s\S]*?)<\/book>/g;
  let bookMatch;
  
  while ((bookMatch = bookRegex.exec(xml)) !== null) {
    const bookId = bookMatch[1];
    const greekTitle = bookMatch[2].trim();
    const bookContent = bookMatch[3];
    
    if (!BOOK_MAPPING[bookId]) {
      console.log(`⚠️  Skipping unknown book: ${bookId}`);
      continue;
    }
    ...
    while ((pMatch = pRegex.exec(bookContent)) !== null) {
      const pContent = pMatch[1];
      
      // Find verse number - format: <verse-number id="Matthew 1:1">1:1</verse-number>
      const verseNumMatch = pContent.match(/<verse-number id="[^"]*\s*(\d+):(\d+)"[^>]*>(?:\d+:\d+)?<\/verse-number>/);
      if (!verseNumMatch) continue;
      
      const chapterNum = parseInt(verseNumMatch[1]);
      const verseNum = parseInt(verseNumMatch[2]);
      
      // Extract verse text - everything after verse-number closing tag
      let verseText = pContent
        .replace(/<verse-number[^>]*>(?:[^<]*)<\/verse-number>/g, '')
        .replace(/<w>([^<]*)<\/w>/g, '$1')
    ...
    // Remove empty chapters at the end
    while (chapters.length > 0 && chapters[chapters.length - 1].length === 0) {
      chapters.pop();
    }
    
    const totalVerses = chapters.reduce((sum, ch) => sum + ch.length, 0);
    ...
    books.push(bookObj);
    console.log(`✓ ${koreanName} (${abbrev}) - ${chapters.length} chapters, ${totalVerses} verses`);
  }
  ...
  const totalVerses = books.reduce((sum, b) => sum + b.chapters.reduce((s, c) => s + c.length, 0), 0);
