...
type Tab = 'users' | 'book-archive' | 'user-archive' | 'upgrade-requests' | 'delete-requests' | 'world-prayer-approval';
...
function AdminDashboardContent() {
  ...
  const [worldPrayerRequests, setWorldPrayerRequests] = useState<any[]>([]);
  ...
  // Load world prayer approval requests
  useEffect(() => {
    if (isAdmin && activeTab === 'world-prayer-approval') {
      loadWorldPrayerRequests();
    }
  }, [isAdmin, activeTab]);
  ...
  // Load pending world prayers
  const loadWorldPrayerRequests = async () => {
    setLoadingApprovals(true);
    try {
      const data = await getPendingWorldPrayers();
      setWorldPrayerRequests(data || []);
    } catch (err) {
      console.error('Error loading world prayer requests:', err);
    } finally {
      setLoadingApprovals(false);
    }
  };

  // Approve world prayer
  const handleApproveWorldPrayer = async (postId: string) => {
    try {
      await approveWorldPrayer(postId);
      alert('세계 기도가 승인되어 게시판에 표시됩니다.');
      await loadWorldPrayerRequests();
    } catch (err) {
      console.error('Error approving world prayer:', err);
      alert('승인 중 오류가 발생했습니다.');
    }
  };

  // Reject world prayer
  const handleRejectWorldPrayer = async (postId: string) => {
    try {
      const supabase = getSupabase();
      // Delete the unapproved world prayer
      const { error } = await supabase.from('reflections').delete().eq('id', postId);
      if (error) throw error;
      alert('세계 기도가 반려되었습니다.');
      await loadWorldPrayerRequests();
    } catch (err) {
      console.error('Error rejecting world prayer:', err);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-stone-200 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex">
            ...
            <button
              onClick={() => setActiveTab('world-prayer-approval')}
              className={`px-4 py-3 text-sm border-b-2 transition-colors ${
                activeTab === 'world-prayer-approval'
                  ? 'border-blue-600 text-blue-600 font-medium'
                  : 'border-transparent text-stone-500 hover:text-blue-600'
              }`}
            >
              <span className="flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                세계 기도 승인
                {worldPrayerRequests.length > 0 && (
                  <span className="bg-blue-100 text-blue-700 text-xs px-1.5 py-0.5 rounded-full">
                    {worldPrayerRequests.length}
                  </span>
                )}
              </span>
            </button>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* World Prayer Approval Tab */}
        {activeTab === 'world-prayer-approval' && (
          <div>
            <h2 className="text-lg font-semibold text-stone-800 mb-4">세계 기도 승인</h2>
            {loadingApprovals ? (
              <div className="flex items-center justify-center py-12">
                <Loader2 className="w-8 h-8 text-stone-300 animate-spin" />
              </div>
              ...
              <div className="text-center py-12 text-stone-400">
                ...
                <p className="text-sm">현재 승인 대기 중인 세계 기도가 없습니다.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {worldPrayerRequests.map((post) => (
                  <div key={post.id} className="border border-blue-200 rounded-lg p-4 bg-blue-50/50">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="text-sm font-medium text-stone-800">
                          {post.profiles?.nickname || post.profiles?.email?.split('@')[0] || '익명'}
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleRejectWorldPrayer(post.id)}
                          className="px-3 py-1.5 text-xs bg-stone-100 text-stone-600 rounded hover:bg-stone-200 transition-colors"
                        >
                          반려
                        </button>
                        <button
                          onClick={() => handleApproveWorldPrayer(post.id)}
                    ...
                    {post.title && (
                      <p className="text-sm font-medium text-stone-800 mb-2">제목: {post.title}</p>
                    )}
                    <div className="bg-white rounded p-3 border border-blue-100">
                      <p className="text-sm text-stone-700 whitespace-pre-wrap line-clamp-5">{post.content}</p>
                    </div>
                  </div>
                ))}
              </div>
